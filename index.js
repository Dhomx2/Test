...
// يستدعي جميع الايديات
const n8a6id = config.n8a6id; // مسؤول النقاط
const staffid = config.staffid; // ادارة
const prefix = config.prefix; // برفكس
const LOG_CHANNEL_ID = config.LOG_CHANNEL_ID; // اللوق العادي
const categoryID = config.categoryID; // كاتاقوري التكت
const pointslog = config.pointslog; // لوق النقاط
const TICKET_LOG_CHANNEL_ID = config.TICKET_LOG_CHANNEL_ID; // لوق التكت
const token = config.token;  // توكن
...
client.on('message', async message => {
    ...
    if (command === `${prefix}زيد`) {
        ...
        // ====== إرسال لوق الإيمبد ======
        const logChannel = message.guild.channels.cache.get(pointslog);
        if (logChannel) {
            const embed = new Discord.MessageEmbed()
                .setTitle('📝 سجل النقاط')
                .setColor('GREEN')
